...
sharing:
  method _plus(self, other :: PPrintDoc) -> PPrintDoc:
  ...
  method pretty(self, width :: Number):
    format(width, self)
  end
end
data Item:
...
#   end
# end
fun collect-concats(i, m, it :: PPrintDoc, rest):
  cases(PPrintDoc) it:
    | concat(fst, snd, _, _) =>
      collect-concats(i, m, fst, collect-concats(i, m, snd, rest))
    | else => link(item(i, m, it), rest)
  end
end
fun format(width :: Number, doc :: PPrintDoc) block:
  var cur-line = empty
  var output = empty
  fun emit-text(s):
  ...
  fun gen-output() block:
    output := link(cur-line, output)
    for lists.fold(lines from empty, line :: List<String> from output):
    ...
    cases(List) items:
      ...
      | link(first, rest) =>
        ...
        cases(PPrintDoc) first.d block:
          | mt-doc(_, _) => process(column, rest)
          | concat(fst, snd, _, _) => process(column, collect-concats(i, m, first.d, rest))
          | str(s, flat-width, _) =>
            emit-text(s)
            process(column + flat-width, rest)
          | blank(n, _, _) =>
            emit-spaces(n)
            process(column + n, rest)
          | align(d, _, _) => process(column, link(item(column, m, d), rest))
          | nest(n, d, _, _) => process(column, link(item(i + n, m, d), rest))
          | hardline(_, _) =>
            if m block:
          ...
          | if-flat(flat, vert, _, _) =>
            process(column, link(item(i, m, if m: flat else: vert end), rest))
          | align-spaces(n, _, _) =>
            if m block:
              process(column, rest)
            else:
          ...
          | group(d, flat-width, has-hardline) =>
            if m: process(column, link(item(i, true, d), rest))
            else if has-hardline: process(column, link(item(i, false, d), rest))
            else if (width - column) >= flat-width:
              # This used to check whether rest fits into the remaining space,
              ...
              process(column, link(item(i, true, d), rest))
            else:
              process(column, link(item(i, false, d), rest))
            end
        end
    end
  end
...
shadow concat = lam(fst :: PPrintDoc, snd :: PPrintDoc): fst + snd end
...
fun flow-map<A>(sep :: PPrintDoc, f :: (A -> PPrintDoc), items :: List<A>) -> PPrintDoc:
...
fun flow(items :: List<PPrintDoc>): flow-map(sbreak(1), lam(x): x end, items) end
fun vert(items :: List<PPrintDoc>): flow-map(hardline, lam(x): x end, items) end
fun parens(d :: PPrintDoc): group(lparen + d + rparen) end
fun braces(d :: PPrintDoc): group(lbrace + d + rbrace) end
fun brackets(d :: PPrintDoc): group(lbrack + d + rbrack) end
str-squote = str("'")
str-dquote = str('"')
fun dquote(s :: PPrintDoc): group(str-dquote + s + str-dquote) end
fun squote(s :: PPrintDoc): group(str-squote + s + str-squote) end

fun hang(i :: Number, d :: PPrintDoc): align(nest(i, d)) end
fun prefix(n :: Number, b :: Number, x :: PPrintDoc, y :: PPrintDoc): group(x + nest(n, sbreak(b) + y)) end
...
fun separate(sep :: PPrintDoc, docs :: List<PPrintDoc>) -> PPrintDoc:
  for fold(acc :: PPrintDoc from mt-doc, d from docs):
    if is-mt-doc(d): acc
...
fun surround-separate(n :: Number, b :: Number, void :: PPrintDoc, open :: PPrintDoc, sep :: PPrintDoc, close :: PPrintDoc, docs :: List<PPrintDoc>):
  cases(List) docs:
    | empty => void
    | link(_, _) => 
      surround(n, b, open, separate(sep, docs), close)
  end
end

fun label-align-surround(label :: PPrintDoc, open :: PPrintDoc, sep :: PPrintDoc, contents :: List<PPrintDoc>, close :: PPrintDoc):
