  ...
  for lists.each(s from stmts):
    cases(A.Expr) s block:
      ...
      | s-data(l, name, params, mixins, variants, shared, _check-loc, _check) =>
        ...
        ann-name = A.s-name(l, name)
        rev-type-binds := link(A.s-newtype-bind(l, ann-name, namet), rev-type-binds)
        rev-stmts := link(A.s-data-expr(l, name, ann-name, namet, params, mixins, variants, shared, _check-loc, _check), rev-stmts)
      | else =>
        rev-stmts := link(s, rev-stmts)
    end
  end
  ...
  cases(List) stmts:
    ...
    | link(f, rest-stmts) =>
      cases(A.Expr) f:
        ...
        | s-data-expr(l, name, ann-name, namet, params, mixins, variants, shared, _check-loc, _check) =>
          ...
          data-expr = A.s-data-expr(l, name, ann-name, namet, params, mixins, variants, shared, _check-loc, _check)
...
fun resolve-names(p :: A.Program, initial-env :: C.CompileEnvironment):
  ...
  names-visitor = A.default-map-visitor.{
    ...
    method s-data-expr(self, l, name, ann-name, namet, params, mixins, variants, shared-members, _check-loc, _check) block:
      ...
      ann-name-handled = handle-ann(l, self.type-env, ann-name)
      result = A.s-data-expr(l, name, ann-name-handled.id, namet, atoms.reverse(),
      ...
      datatypes.set-now(ann-name-handled.id.key(), result)
      result
    end,
