...
const ListSnoc = E.Rule.ListSnoc;
...
g.addRule("Configs", [new Nonterm("Configs"), new Nonterm("Config")], ListSnoc("Configs", "Config"));
g.addRule("Configs", [new Nonterm("Config")]);
g.addRule("Config", [new Token("NAME"), new Token("COLON"), new Token("NAME")]);
g.addRule("Rules", [new Nonterm("Rules"), new Nonterm("Rule")], ListSnoc("Rules", "Rule"));
g.addRule("Rules", [new Nonterm("Rule")]);
g.addRule("Rule", [new Token("NAME"), new Token("COLON"), new Nonterm("Alts")]);
g.addRule("Alts", [new Nonterm("Alts"), new Token("BAR"), new Nonterm("Alt")], ListSnoc("Alts", "Alt"));
...
g.addRule("Flags", [new Nonterm("Flags"), new Token("COMMA"), new Nonterm("Flag")], ListSnoc("Flags", "Flag"));
g.addRule("Flags", [new Nonterm("Flag")]);
g.addRule("Flag", [new Token("NAME"), new Token("COLON"), new Token("NAME")]);
g.addRule("Items", [new Nonterm("Items"), new Nonterm("Item")], ListSnoc("Items", "Item"));
g.addRule("Items", []);
...
function generateAlts(ruleName, alts, shouldInline) {
  ...
  if (alts.kids.length === 1) {
    ...
    if (shouldInline) {
      ...
      if (gen.flags)
        ret.rules.push("g.addRule(" + JSON.stringify(ruleName) + ", [" + gen.rhs + "], E.Rule.defaultAction, "
                       + JSON.stringify(gen.flags) + ")");
      else
...
function generateItem(ruleName, item) {
  if (item.name === "Name") {
    ...
    ret.rules.push("g.addRule(" + json_newNameStar + ", " +
                   "[new Nonterm(" + json_newNameStar + ")" + ", new Nonterm(" + json_newNameOne + ")], " +
                   "E.Rule.ListSnoc(" + json_newNameStar + ", " + json_newNameOne + ", true));");
    ...
    ret.rules.push("g.addRule(" + json_newNamePlus + ", " +
                   "[new Nonterm(" + json_newNamePlus + ")" + ", new Nonterm(" + json_newNameOne + ")], " +
                   "E.Rule.ListSnoc(" + json_newNamePlus + ", " + json_newNameOne + ", true));");
    if (item.kids[0].name === "Alts") {
      var gen = generateAlts(newNameOne, item.kids[0], true);
      ret.rules = ret.rules.concat(gen.rules);
    } else {
...
if (parsed !== undefined) {
  ...
  var jglrRunPath = process.argv[4];
  ...
  out.write("\n\n");
  out.write("g.initializeParser(false);\n");
  if (bnfJS.config.sortBy) {
    out.write("  g.sortAmbiguitiesBy(" + JSON.stringify(bnfJS.config.sortBy) + ");\n");
  }
